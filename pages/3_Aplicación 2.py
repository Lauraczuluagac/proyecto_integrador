...
for button in botones:
    tk.Button(ventana, text=button, padx=20, pady=20, command=lambda b=button: entrada.insert('end', b) if b != '=' else calcular()).grid(row=row_val, column=col_val)
    col_val += 1
    if col_val > 3:
        col_val = 0
        row_val += 1
...
# Botón Clear
tk.Button(ventana, text='Clear', padx=20, pady=20, command=lambda: entrada.delete(0, 'end')).grid(row=row_val, column=col_val)
